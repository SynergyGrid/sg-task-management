...
- WhatsApp importer (OpenRouter-powered) that reads the last 30 days of chat history, extracts action items, and drops them into the "GENERAL -> General Project" board automatically.
...
1. **Netlify / Cloudflare Pages (recommended)** - the repo already contains `netlify.toml`. Set the base directory to `client`, build command `npm run build`, and publish directory `dist`. Add the following environment variables:
   ...
   - `VITE_OPENROUTER_MODEL` (optional, defaults to `openai/gpt-4o-mini`)
...
3. Pick the OpenRouter model in the dropdown if you need to override the default (`VITE_*` drives the list).
4. OpenRouter calls your selected model, extracts action items, respects any explicit or strongly implied deadlines, and only assigns tasks if the named person already exists as a member.
...
> Tip: Enter model ids exactly as OpenRouter lists them (for example `openai/gpt-4o-mini`). The dropdown shows a few good defaults, but any compatible model can be pasted into the field.

Environment variables control the behaviour and can be overridden per deployment: `VITE_OPENROUTER_MODEL`, `VITE_WHATSAPP_LOOKBACK_DAYS`, `VITE_WHATSAPP_COMPANY_NAME`, `VITE_WHATSAPP_PROJECT_NAME`, `VITE_WHATSAPP_MAX_LINES`, and the optional `VITE_WHATSAPP_LOG_SHEET_ID` for future spreadsheet logging.
...
- The WhatsApp importer needs OpenRouter credentials and a destination company/project (see `VITE_*` variables above).
