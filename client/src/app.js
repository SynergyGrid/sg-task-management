...
const USERGUIDE_CANONICAL_HEADINGS = new Map([
  ["workspace overview", "## Workspace Overview"],
  ["main views", "## Main Views"],
  ["sections & boards", "## Sections & Boards"],
  ["creating & editing tasks", "## Creating & Editing Tasks"],
  ["meetings & emails", "## Meetings & Emails"],
  ["people & departments", "## People & Departments"],
  ["finding & monitoring work", "## Finding & Monitoring Work"],
  ["activity & history", "## Activity & History"],
  ["userguide panel", "## Userguide Panel"],
  ["exports & housekeeping", "## Exports & Housekeeping"],
  ["whatsapp chat importer", "## WhatsApp Chat Importer"],
  ["maintaining this guide", "## Maintaining This Guide"],
  ["recent improvements", "## Recent improvements"],
]);

const isHeadingLine = (line) => /^#{2,3}\s/.test(line);
const isBulletLine = (line) => /^-+\s/.test(line) || /^\d+\.\s/.test(line);

const normaliseGuideLines = (lines) => {
  if (!Array.isArray(lines)) return [...DEFAULT_USERGUIDE_LINES];
  const seen = new Set();
  const formatted = [];
  lines.forEach((raw) => {
    let value = typeof raw === "string" ? raw.trim() : "";
    if (!value) return;
    const plain = value.replace(/^#+\s+/, "").toLowerCase();
    if (USERGUIDE_CANONICAL_HEADINGS.has(plain)) {
      value = USERGUIDE_CANONICAL_HEADINGS.get(plain);
    }
    let line = value;
    if (isHeadingLine(value)) {
      line = value;
    } else if (isBulletLine(value)) {
      line = value.replace(/^-+\s*/, "- ").replace(/\s+/g, " ").trim();
    } else {
      line = `- ${value}`;
    }
  ...
  return formatted.length ? formatted : [...DEFAULT_USERGUIDE_LINES];
};

const DEFAULT_USERGUIDE = normaliseGuideLines(DEFAULT_USERGUIDE_LINES);
const USERGUIDE_LATEST_ENTRIES = [
  "## Main Views",
  "- List ↔ Board toggle – every project remembers whether you prefer the list or kanban-style board.",
  "## Creating & Editing Tasks",
  "- Task cards surface the first three lines of notes plus assignee, action items, and due dates at a glance.",
  "- Use the Mark completed / Restore control inside the task dialog to switch status.",
  "- Closing any task, meeting, or email dialog auto-saves your changes.",
...
const normaliseUserguide = (entries) => {
  ...
  .filter(Boolean);
  if (!cleaned.length) return [...DEFAULT_USERGUIDE];
  return normaliseGuideLines(cleaned);
};
...
const ensureUserguideHasLatestEntries = () => {
  ...
  const canonical = normaliseGuideLines(state.userguide);
  if (canonical.length !== state.userguide.length || canonical.some((line, index) => line !== state.userguide[index])) {
    state.userguide = canonical;
    mutated = true;
  }
  if (!mutated) return;
  saveJSON(STORAGE_KEYS.userguide, state.userguide);
...
const renderFilterBar = () => {
  ...
  const assignFilterValue = (key, value) => {
    state.filters = { ...state.filters, [key]: value };
  };
